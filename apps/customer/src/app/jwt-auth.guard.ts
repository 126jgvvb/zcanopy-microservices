  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    try {
      if (devBypass && segments.length === 3) {
        ...
        this.logger.log(`JwtAuthGuard: bypassed verification for email=${payload.email}, customerId=${payload.customerId}`);
        request.user = payload;
        return true;
      }

      const payload = this.jwtService.verify(token);
      this.logger.log(`JwtAuthGuard: token verified for email=${payload.email}, customerId=${payload.customerId}`);
      request.user = payload;
    } catch (err) {
