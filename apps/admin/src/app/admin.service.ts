...
export class AdminService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.subscriber.on('message', async (channel, message) => {
      if (channel === 'update_platform_commission') {
        try {
          console.log(`The raw message is: ${message}`);

          const raw = JSON.parse(message);
          const data = raw.data || raw;

          console.log(`The decoded payload is: ${JSON.stringify(data)}`);
      ...
      } else if (channel === 'support_message_received') {
        try {
          const raw = JSON.parse(message);
          const data = raw.data || raw;
          await this.handleSupportMessageReceived(data);
        } catch (error) {
          this.logger.error(`Failed to handle support message received: ${(error as Error).message}`);
        }
      }
    });

    this.subscriber.subscribe('support_message_received', (err) => {
      if (err) {
        console.error('Failed to subscribe to support_message_received', err);
      }
    });

    this.logger.log('Subscribed to update_platform_commission, notifications_report, broker_feedback_received, and support_message_received Redis channels');
  }
  ...
  private async handleUpdatePlatformCommission(amount: number): Promise<void> {
    try {
      ...
      if (!Number.isFinite(numericAmount) || numericAmount < 0) {
        this.logger.warn(`Invalid platform commission amount received: ${amount}, skipping update`);
        return;
      }

      const dashboard = await this.getOrCreateDashboard();
      const current = Number(dashboard.currentCommission) || 0;
      dashboard.currentCommission = current + numericAmount;
      await this.dashboardRepo.save(dashboard);
      this.logger.log(`Updated platform commission: ${current} + ${numericAmount} = ${dashboard.currentCommission}`);

      const queryRunner = this.dashboardRepo.manager.connection.createQueryRunner();
      await queryRunner.connect();
      await queryRunner.startTransaction();
      try {
        const existing = await queryRunner.manager.getRepository('PlatformRevenue').findOne({ where: {} });
        if (existing) {
          await queryRunner.manager.getRepository('PlatformRevenue').update(existing.id, {
            lastRevenue: existing.currentRevenue,
            currentRevenue: dashboard.currentCommission,
          });
        } else {
          await queryRunner.manager.getRepository('PlatformRevenue').insert({
            currentRevenue: dashboard.currentCommission,
            lastRevenue: dashboard.currentCommission,
            totalTransactions: 1,
          });
        }
      ...
      this.logger.log(`Updated platform revenue to match platform commission: ${dashboard.currentCommission}`);
    } catch (err) {
  ...
  private async handleSupportMessageReceived(data: {
    id: number;
    ...
    try {
      ...
      dashboard.clientMessages = [...(dashboard.clientMessages ?? []), entry];
      await this.dashboardRepo.save(dashboard);
      this.logger.log(`Appended support message id=${data.id} from ${data.customerEmail} to dashboard clientMessages`);
    } catch (err) {
      this.logger.error(`Failed to handle support message received: ${(err as Error).message}`);
    }
  }
  ...
  async getCurrentCommission() {
    try {
      const dashboard = await this.getOrCreateDashboard();
      const currentCommission = Number(dashboard.currentCommission) || 0;
      const bookingCommission = Number(dashboard.bookingCommission) || 0;

      return {
        currentCommission,
        platformCommission: currentCommission,
        bookingCommission,
        totalEarnings: currentCommission + bookingCommission,
      };
    } catch (err) {
  ...
  async sendMessage(dto: {
    ...
    try {
      ...
      this.logger.log(`Admin message saved with id=${saved.id}`);

      if (dto.channel === 'email' && dto.recipientEmail) {
        this.logger.log(`Emitting send_admin_message_email to ${dto.recipientEmail}`);
        this.redisClient.emit('send_admin_message_email', {
          recipientEmail: dto.recipientEmail,
          recipientName: dto.recipientName,
          subject: dto.subject,
        ...
        this.logger.log(`Emitting send_admin_message_sms to ${dto.recipientPhone}`);
