...
export class AdminController {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Get a single property by id (admin)' })
  async getProperty(@Param('propertyId') propertyId: string) {
    this.logger.log(`Admin get property request for ${propertyId}`);
    const result = await this.proxyService.forwardToProperty('GetProperties', {
      id: propertyId,
      page: 1,
      limit: 1,
    });
    const property = (result as any)?.properties?.[0] ?? null;
    return { property };
  }
  ...
  @Post('google')
  @ApiOperation({ summary: 'Admin Google sign-in (returns base64 token)' })
  async googleLogin(@Body() body: { idToken: string }) {
    this.logger.log('Admin Google sign-in request');
    return this.proxyService.forwardToAdmin('GoogleLogin', { idToken: body.idToken });
  }
  ...
  @Get('comments')
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Get all property comments for admin' })
  async getAllComments(@Query() query: any) {
    this.logger.log(`Get all comments request: ${JSON.stringify(query)}`);
