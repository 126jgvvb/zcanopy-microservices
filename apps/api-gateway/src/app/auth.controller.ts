...
export class AuthController {
  ...
  @Post('broker/google')
  async brokerGoogleLogin(@Body() dto: any) {
    this.logger.log('Broker Google login request via gateway');
    return this.proxyService.forwardToAuth('LoginBrokerGoogle', dto);
  }
...
export class UsersController {
  ...
  @ApiOperation({ summary: 'Get broker subscription details (tier, expiry and limits)' })
  async getSubscriptionDetails(@Query('brokerCode') brokerCode: string) {
    this.logger.log(`Get subscription details for ${brokerCode}`);
