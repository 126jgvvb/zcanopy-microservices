...
import { SupportMessageEntity } from '../entitty/support-message.entity';
import {
  otpEmailHtml,
  passwordResetEmailHtml,
  passwordChangedEmailHtml,
  adminAlertEmailHtml,
  brokerAccountCreatedEmailHtml,
...
export class NotificationService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    private readonly supportMessageRepo: Repository<SupportMessageEntity>,
  ...
  async onModuleInit() {
    ...
    this.subscriber.on('error', (err: Error) =>
      this.logger.error(`Subscriber Redis connection error: ${err.message}`),
    );
    ...
    this.subscriber.subscribe('send_property_payment_invoice', (err) => {
      if (err) {
        console.error('Failed to subscribe to send_property_payment_invoice', err);
      }
    });
    ...
    this.subscriber.on('message', async (channel, message) => {
      ...
      } else if (channel === 'send_property_payment_invoice') {
        try {
          const raw = JSON.parse(message);
          const data = raw.data || raw;
          this.logger.log(`Sending property payment invoice for transactionCode=${data.transactionCode}`);
          ...
          if (data.customerEmail) {
            await this.sendPropertyPaymentEmail({
          ...
          if (data.customerPhone) {
            await this.sendPropertyPaymentSms({
  ...
  async sendPaymentEmail(payload: PaymentNotificationPayload) {
    try {
      ...
      const body = paymentConfirmationEmailHtml({ username: payload.username, email: payload.email, invoice: payload.invoice });
      const result = await this.dispatchEmail(payload.email, subject, body);
  ...
  async sendPropertyPaymentEmail(payload: {
    ...
    try {
      ...
      const body = propertyPaymentConfirmationEmailHtml({ username: payload.username, email: payload.email, invoice: payload.invoice });
      const result = await this.dispatchEmail(payload.email, subject, body);
      await this.saveNotification({ type: 'payment', channel: 'email', title: subject, content: body, recipient: payload.email, result });
      return this.result('email', payload.email);
    } catch (err) {
  ...
  async sendAdminMessage(payload: {
    ...
    try {
      if (payload.channel === 'email' && payload.recipientEmail) {
        const subject = payload.subject || 'Message from Admin';
        const html = adminAlertEmailHtml({ subject, message: payload.body, recipientName: payload.recipientName });
        const result = await this.dispatchEmail(payload.recipientEmail, subject, html);
  ...
  async sendBrokerApprovalEmail(payload: BrokerApprovalPayload) {
    try {
      ...
      const body = brokerApprovalEmailHtml({ username: payload.username, email: payload.email, brokerCode: payload.brokerCode });
      const result = await this.dispatchEmail(payload.email, subject, body);
  ...
  async sendBrokerCreated(payload: BrokerCreatedPayload) {
    try {
      ...
      const body = adminAlertEmailHtml({ subject, message: `New broker ${payload.username} (${payload.email}) signed up and is awaiting approval.`, recipientName: 'Admin' });
  ...
  async sendBrokerCodeCreated(payload: { email: string; username: string; brokerCode: string }) {
    try {
      ...
      const body = brokerAccountCreatedEmailHtml({ username: payload.username, email: payload.email, brokerCode: payload.brokerCode });
      const result = await this.dispatchEmail(payload.email, subject, body);
  ...
  async sendPaymentFailed(payload: PaymentFailedPayload) {
    try {
      ...
      const body = adminAlertEmailHtml({ subject, message: `Payment for ${payload.tier} tier failed for broker ${payload.username}: ${payload.message}`, recipientName: 'Admin' });
      const result = await this.dispatchEmail('admin@zcanopy.com', subject, body);
  ...
  async sendBrokerLoginNewDevice(payload: BrokerLoginNewDevicePayload) {
    try {
      ...
      const body = adminAlertEmailHtml({ subject, message: `Hi ${payload.username}, a new device login was detected for your broker account (${payload.brokerCode}). If this was not you, please secure your account immediately.`, recipientName: payload.username });
      const result = await this.dispatchEmail(payload.email, subject, body);
  ...
  async getSupportMessages(query: { page?: number; limit?: number; status?: string; customerEmail?: string }) {
    try {
      const page = Number(query.page) || 1;
      const limit = Number(query.limit) || 20;
      ...
      const [messages, total] = await this.supportMessageRepo.findAndCount({
        where,
      ...
      this.logger.error(`Failed to get support messages: ${(err as Error).message}`);
      throw err;
    }
  }

  async recordSupportMessage(data: {
    customerEmail: string;
    subject?: string;
    textContent?: string;
    htmlContent?: string;
