...
import { BrokerDto, RequestOtpDto, ResendOtpDto, LoginBrokerDto, CreateBrokerSessionDto, GetBrokerSessionsDto, RevokeBrokerSessionDto, GetBrokerByCodeDto, UpdateBrokerSettingsDto, GetAvailableTiersDto, SubmitBrokerFeedbackDto, GetBrokerMessagesDto, LogoutBrokerDto, UnsubscribeBrokerDto, RequestUnsubscribeOtpDto, SetupBrokerAccountDto, SearchBrokersDto, ValidateBrokerDto, GetBrokerByIdDto, GetSubscriptionDetailsDto, SaveUserInfoDto, UpdateUserFieldDto, RegisterBrokerDto, SendBrokerOtpDto, VerifyBrokerOtpDto } from './dtos/broker-dto';
...
@Inject('REDIS_CLIENT') private readonly redisClient:ClientProxy,
    ...
    async registerBroker(dto: RegisterBrokerDto) {
        try {
            ...
            this.logger.log(`Emitted send_email_otp for ${dto.email}`);
            ...
            this.logger.log(`Emitted send_sms_otp for ${dto.phoneNumber}`);

            this.logger.log(`Broker registration OTPs for ${dto.email}: email OTP=${emailOtp}, phone OTP=${phoneOtp}`);
    ...
    async sendBrokerOtp(dto: SendBrokerOtpDto) {
        try {
            ...
            const session = await this.redis.get(this.registrationSessionKey(dto.email));
            if (!session) {
    ...
    async verifyBrokerOtp(dto: VerifyBrokerOtpDto) {
        try {
            ...
            const sessionRaw = await this.redis.get(this.registrationSessionKey(dto.email));
            if (!sessionRaw) {
            ...
            await this.redis.del(this.registrationSessionKey(dto.email));
    ...
    */
    ...
    async processSubscriptionPayment(dto: { phoneNumber?: string; tier: string; brokerId: string }) {
        const broker = await this.brokerRepo.findOne({
            where: [{ id: dto.brokerId }, { brokerCode: dto.brokerId }],
        });
        if (!broker) {
    ...
    async submitVerificationDocuments(dto: { brokerCode: string; idFrontUrl: string; idBackUrl: string }) {
        try {
            ...
            const ninImages = [dto.idFrontUrl, dto.idBackUrl].filter(Boolean);
            if (ninImages.length < 2) {
                return {
                    success: false,
                    message: 'Both ID front and back images are required',
                };
            }
            ...
            this.logger.log(`Broker ${dto.brokerCode} submitted verification documents`);
            ...
            this.logger.error(`Failed to submit verification documents for ${dto.brokerCode}:`, err);
            throw err;
        }
    }
    ...
    async withdraw(dto: {
      ...
      try {
        const brokerCode = dto.brokerCode;
        if (!brokerCode) {
        ...
        if (!broker.isVerified) {
          return {
            success: false,
            message: 'Identity verification required. Please verify your documents before withdrawing funds.',
          };
        }
        ...
        const currentBalance = Number(broker.walletBalance || 0);
        if (withdrawalAmount > currentBalance) {
          throw new BadRequestException(`Insufficient wallet balance. Available: ${currentBalance}, requested: ${withdrawalAmount}`);
        }

        this.logger.log(`Broker withdraw request: amount=${withdrawalAmount}, phone=${dto.phoneNumber}, broker=${brokerCode}, balanceBefore=${currentBalance}`);
        ...
        const isSuccess = result?.status === 'Success' || result?.success || result?.status ==='Scheduled';
        if (isSuccess) {
        //  const netAmount = Number(result?.netAmount || result?.amount || withdrawalAmount);
        const netAmount = Number(withdrawalAmount);  
        broker.walletBalance = Math.max(0, currentBalance - netAmount);
          ...
          const payout = this.payoutsRepo.create({
            ...
            customerName: dto.payeeName || broker.username,
            grossAmount: netAmount,
            platformCommission: 0,
            bookingCommission: 0,
            netAmount,
            transactionID: result?.transactionId || result?.referenceNumber || '',
            transactionCode: result?.referenceNumber || '',
            payoutStatus: 'SUCCESS',
            provider: dto.provider?.toLowerCase() || 'mobile_money',
            recipient_phone: dto.phoneNumber,
          });

          await this.payoutsRepo.save(payout);
          this.logger.log(`Broker withdrawal recorded: payoutId=${payout.id}, newBalance=${broker.walletBalance}`);
        }
    ...
    async getWalletTransactions(dto: { brokerCode: string; page: number; limit: number }) {
        try {
            ...
            this.logger.error(`Failed to get wallet transactions for broker ${dto.brokerCode}:`, err);
            throw err;
        }
    }
    ...
    async resendOtp(dto: ResendOtpDto) {
        try {
            ...
            const purpose = dto.purpose || 'broker-registration';
    ...
    private comparePassword(stored: string, input: string): boolean {
    ...
    async loginBroker(dto: LoginBrokerDto) {
        try {
            ...
            if (googleId) {
                ...
                if (!this.comparePassword(broker.password, password)) {
            ...
            await this.redis.sadd(`broker:sessions:${broker.brokerCode}`, sessionId);
            await this.redis.expire(`broker:sessions:${broker.brokerCode}`, ttl);
    ...
    async createBrokerSession(dto: CreateBrokerSessionDto) {
        try {
            ...
            await this.redis.set(`broker:session:${sessionId}`, JSON.stringify(sessionData), 'EX', ttl);
            await this.redis.sadd(`broker:sessions:${dto.brokerCode}`, sessionId);
            await this.redis.expire(`broker:sessions:${dto.brokerCode}`, ttl);
    ...
    async getBrokerSessions(dto: GetBrokerSessionsDto) {
        try {
            ...
            const sessionIds = await this.redis.smembers(`broker:sessions:${dto.brokerCode}`);
            const sessions: any[] = [];
    ...
    async revokeBrokerSession(dto: RevokeBrokerSessionDto) {
        try {
            ...
            await this.redis.del(`broker:session:${dto.sessionId}`);
    ...
    async setupBrokerAccount(dto: SetupBrokerAccountDto) {
        try {
            ...
            await this.redis.expire(`broker:sessions:${updated.brokerCode}`, ttl);
    ...
    async validateBroker(dto: ValidateBrokerDto) {
        try {
            if (!dto?.email || !dto?.password) {
                this.logger.warn(`validateBroker missing credentials: email=${dto?.email}`);
                return {
                    success: false,
    ...
    async getSubscriptionDetails(dto: GetSubscriptionDetailsDto) {
        try {
            ...
            this.logger.error(`Failed to get subscription details for broker ${dto.brokerCode}:`, err);
            throw err;
        }
    }
    ...
    private async getBrokerCache(brokerCode: string): Promise<any> {
        try {
            const cached = await this.redis.get(`broker:cache:${brokerCode}`);
            if (cached) {
    ...
    private async setBrokerCache(brokerCode: string, broker: BrokerEntity): Promise<void> {
        try {
            ...
            await this.redis.set(`broker:cache:${brokerCode}`, payload, 'EX', 300);
            await this.redis.set(`broker:cache:${broker.id}`, payload, 'EX', 300);
        } catch {
    ...
    private async invalidateBrokerCache(brokerCode: string): Promise<void> {
        try {
            await this.redis.del(`broker:cache:${brokerCode}`);
            const broker = await this.brokerRepo.findOne({ where: { brokerCode } });
            if (broker) {
                await this.redis.del(`broker:cache:${broker.id}`);
            }
        } catch {
